...
#define the_topic1 "purchase_topic_1"


//define your default values here, if there are different values in config.json, they are overwritten.
char mqtt_server[40] = "104.236.210.175";
...
void setup() {
  ...
  if (!wifiManager.autoConnect("GrabYerAmazonLink", "password")) {
...
void reconnect() {
    ...
    while (!client.connected()) {
        ...
        // Attempt to connect
        if (client.connect("TestMQTT1")) { //* See //NOTE below
...
client.setServer(the_mqtt_server, 1883);
 ...
 client.loop();
  ...
  String message_to_send = String("{") + String("\"OrderInfo\"") + String(":[{\"UserName\":\"") +  String(mqtt_server) + "\", \"AmazonLink\":\"" + String(mqtt_port) + "\"}]}";
  Serial.println(message_to_send);

  delay(500);
  pubMQTT(the_topic1,message_to_send);
  delay(40000);
